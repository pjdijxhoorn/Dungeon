...
def get_profile(db: Session, profile_id: int):
    ...
    return profile
#
def create_profile(profile, db: Session):

    bmi = bmi_calculation(profile.height_in_m, profile.weight_in_kg)
    age = calculate_age(profile.date_of_birth)  
    max_heart_frequency = max_heart_frequency_calculation(age)
    reserve_heart_frequency = reserve_heart_frequency_calculation(max_heart_frequency, profile.rest_heart_frequency)

    db_Profile = Profile(
                         bmi = bmi,
                         weight_in_kg = profile.weight_in_kg,
                         height_in_m = profile.height_in_m,
                         date_of_birth = profile.date_of_birth,
                         max_heart_frequency = max_heart_frequency,
                         rest_heart_frequency = profile.rest_heart_frequency,
...
def patch_profile(profile_id: int, update_profile, db: Session):
    ...
    age = calculate_age(profile.date_of_birth)
    max_heart_frequency = max_heart_frequency_calculation(age)
    reserve_heart_frequency = reserve_heart_frequency_calculation(max_heart_frequency, update_profile.rest_heart_frequency)
...
def bmi_calculation(height_in_m, weight_in_kg):
    ...
    bmi = weight_in_kg/(height_in_m**2)                                  #Momenteel alleen voor mannen, gezien BMI berekening voor vrouwen anders is, dit willen we in de toekomst ook zeker nog implemeteren maar voor nu hebben we gekozen om het simpeler te houden
    return round(bmi,2)
...
def calculate_age(date_of_birth):
    try:
        ...
        # Calculate the age
        age = current_date.year - date_of_birth_split.year - ((current_date.month, current_date.day) < (date_of_birth_split.month, date_of_birth_split.day))
        return age
